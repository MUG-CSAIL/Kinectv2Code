...
//structure to store info from our one client
...
BYTE fragmentNum = 0; //Which fragment of the current frame we're about to sendsequenceNum = 0; //for bookkeeping, the frame number in our infinite sequence, ranges from 0-255 then loops.
...
int UDPServer::createFakeImage(char * imagePointer){
...
void UDPServer::sendTestImageStream()
{
...
void UDPServer::sendKinectImage(UINT16 *pBuffer){
	if (serverSocket == INVALID_SOCKET) //Make sure we have a valid socket.
	{
		fprintf(stderr, "Internal socket is invalid.\n");
		WSACleanup();
		exit(-1);
	}

	executeMainLoop(pBuffer);

}

BYTE * UDPServer::convertBufferToImage(UINT16 * pBuffer){
	for (int i = 0; i < PBUFFER_SIZE; i++){
		UINT16 pixel = pBuffer[i];
		BYTE lowByte = (BYTE)pixel;
		BYTE highByte = (BYTE)((pixel >> 8) & 0xff);
		int byteIndex = i * 2;
		byteImage[byteIndex] = highByte;
		byteImage[byteIndex+1] = lowByte;
	}
	return byteImage;
}
...
void UDPServer::executeMainLoop(UINT16* pBuffer){
	int sent = 0; //how many bytes we just sent with our latest attempt at transmission
	int sentTotal = 0; //how many bytes we have sent in total through transmission
	int dataLeft = FRAME_SIZE; //how much data is left before the current image has been completely transmitted
	int failCount = 0; //number of failed attempts before we give up and disconnect. 
	int tolen = sizeof(to); //calculate size, we'll use this later.
	BYTE packet[PACKET_SIZE]; //buffer where we'll put messages.
	ZeroMemory(packet, sizeof(packet));
	BYTE * packetPointer = &packet[0];
	BYTE * image = convertBufferToImage(pBuffer);
	BYTE * imagePointer = &image[0];
	int remainingSize = sizeof(packet) - sizeof(sequenceNum) - sizeof(fragmentNum); //Since sizeof(char) is defined to be 1, this is actually sizeof(packet) - 2.
	...
	while (1)
	{
		...
		packetPointer += sizeof(fragmentNum); //move past the fragment number
		memcpy(packetPointer, imagePointer, remainingSize); //copy the packet's remaining capacity of data starting from pointer location
		sent = sendto(serverSocket, (const char*)packet, sizeof(packet), 0, (sockaddr*)&to, tolen); //try to send the packet
		if (sent == SOCKET_ERROR){
			failCount += 1;
			if (failCount > 10){
				printf("Failed 10 times. Shutting down.");
		...
		while (sentTotal < FRAME_SIZE) //until we finish sending the image, repeat the above with one slight change
		{
			...
			packetPointer += sizeof(fragmentNum); //increment pointer
			memcpy(packetPointer, imagePointer, min(remainingSize, dataLeft)); //put image data in rest of packet
			sent = sendto(serverSocket, (const char*)packet, sizeof(packet), 0, (sockaddr*)&to, tolen); //try to send the rest of the image
			if (sent == SOCKET_ERROR)
			{
				failCount += 1;
				if (failCount > 10)
		...
		dataLeft = sizeof(image); //reset how much data is left to send.
		packetPointer = &packet[0]; //lastly, reset packet pointer
		break; //stop loop so we can get invoked with a new pBuffer
	}
}
